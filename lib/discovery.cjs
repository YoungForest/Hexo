...
function buildCatalog(manifest, posts, en = false) {
  ...
  const labels = en ? {
    ...
    reading:'Reading & screen', 'year-review':'Year in review', algorithms:'Algorithms & contests', other:'Other writing', related:'Read next'
  } : {
    ...
    reading:'书影音笔记', 'year-review':'年度总结', algorithms:'算法与竞赛', other:'其他文章', related:'延伸阅读'
  };
  const resolve = id => {
  ...
  for(const [id,p] of byId) {
    ...
    // Capture the author's opt-out before NeXT merges its theme defaults into page locals.
    m.tocDisabled=p.toc===false || p.toc?.enable===false;
  }
  assert(manifest.related_reading===undefined || Array.isArray(manifest.related_reading),'Related reading must be an array');
  const relatedSources=new Set();
  const related=(manifest.related_reading||[]).map(entry=>{
    assert(entry && typeof entry==='object','Invalid related reading entry');
    const source=resolve(entry.post);
    assert(!relatedSources.has(source.id),'Duplicate related source: '+source.id);relatedSources.add(source.id);
    assert(Array.isArray(entry.next) && entry.next.length>=1 && entry.next.length<=2,'Related reading needs one or two targets');
    assert.equal(new Set(entry.next).size,entry.next.length,'Duplicate related target');
    const next=entry.next.map(id=>{
      assert(id!==source.id,'Related reading self-link');
      const target=resolve(id);assert(target.description,'Related target needs existing description: '+id);
      return target;
    });
    // Resolve and validate every entry, but annual navigation always takes precedence.
    if(!metadata.get(source.id).series)metadata.get(source.id).related=next;
    return {post:source.id,next};
  });
  return {reading,series,related,metadata,labels,kinds:KINDS,en};
}
